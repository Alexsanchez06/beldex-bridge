...
import {
  getInfo, getSwaps, swapToken, finalizeSwapToken,
   sendTransactionHash
  
} from "../../store/swapReducer";
import {selectInfo, selectSwaps,selectBalance, selectUnconfirmedBeldexTxs, selectSwapError, selectSwapLoading} from "../../store/swapSelector";
...
const classes = useStyles();
  ...
  console.log('apiHit 7 -->','info -->',info,'swaps -->',swaps,'totalbalance -->',totalbalance)
  ...
  const mergedSwaps = useMemo(() => {
    ...
    console.log('swapsswapsswaps -->',swaps,[...unconfirmedSwaps, swaps],validSwap)
 ...
 const connectToMetaMask = async () => {
  ...
  const connectToBinance = async () => {
    ...
    try {
      if (!mobileView && !window.ethereum.isMetaMask) {
        return props.showMessage(props.t("Trustwallet is not installed."), "error");
      }
 ...
 const  makeTransaction = () => {
    ...
    web3Obj.eth
      ...
      .on("error", (error) => {
        dispatch( sendTransactionErrorLog( {reqObj: { ...options, error: error?.code ? error : error.message }}));
        if (error?.code === 4001) {
          props.showMessage(
            props.t("transactionSignatureError"),
            "error"
          );
        } else {
...
const renderSelection = useCallback((totalSupply, movedBalance) => {
    ...
    <Grid container className={classes.registerWrapper}>
      ...
      <Grid size={{ xs: 12, md: 7 }} >  
        ...
        <Popup
          ...
          open={showPopup}
          onClose={v => {handlePopupClose(v); console.log('closeConsole2', v)}}
        />
  ...
  // Info page
  const renderInfo = useCallback((movedBalance, totalSupply) => {
    ...
    return (
      <Box className={classes.dashBoard}>
        ...
        >
          {/* SVG left arrow */}
          <svg width="20" height="20" className={classes.backImg} viewBox="0 0 24 24">
